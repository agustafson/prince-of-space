...
On this corpus, **strict** (fixed-point) Prince of Space is slower than Eclipse JDT and somewhat slower than Google Java Format and Palantir; **fast** single-pass is in the same ballpark as those JVM formatters. The default engine prioritises idempotency (Rule 1) over raw throughput. In practice, Spotless typically formats only changed files, which keeps wall time small.
...
Throughput on the [Spring Framework](https://github.com/spring-projects/spring-framework) corpus (**c5d7908**, **9213** files, same path filters as the external eval harness). **Avg ms / file** is total JVM wall time for the run divided by file count. Each JVM formatter runs in one process with warmup; JVM: **21.0.11+10-LTS**
...
|-----------|-----------------|----------|
| Prince of Space (strict, fixed-point) | 6.20 | 0 |
| Prince of Space (fast, single pass) | 2.47 | 0 |
| Google Java Format (AOSP) | 2.90 | 0 |
| Palantir Java Format (AOSP) | 2.87 | 0 |
| Eclipse JDT (Spotless default) | 2.40 | 0 |
