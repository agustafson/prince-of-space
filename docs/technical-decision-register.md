...
### TDR-021: Trailing block-lambda keeps `() -> {` on the call line
- **Date:** 2026-04-28
- **Status:** Accepted
- **Decision:** When the **last argument** of a wrapped method/constructor call is a **block-bodied lambda**, keep the leading arguments and the lambda header (`() -> {`, `(a, b) -> {`, etc.) on the call line, let the lambda body wrap with its own block indent, and place the closing `})` on a single line aligned to the call's indent column — regardless of `closingParenOnNewLine`. This holds even when the resulting opener line slightly exceeds `lineLength`. Falls back to standard wrapping only when a leading argument itself carries a leading line/block comment, or another leading argument is itself a block lambda (multi-block-lambda calls remain ambiguous and use the regular per-arg break path).
- **Rationale:** Other mainstream formatters — palantir-java-format, Prettier, ktlint, and Google's style for Kotlin trailing-lambda — uniformly treat a trailing lambda as the "expanded" argument and keep its header inline so the body reads as a natural block. The previous "one arg per line" wrap path placed `() ->` on its own continuation line, which breaks the visual coupling between the call and the lambda body and reads worse on any non-trivial line. Soft overflow on the opener line is the standard tradeoff in those formatters because the alternative is uglier per-arg breakage.
- **Consequences:** Wrapped multi-arg calls with a trailing block lambda now render in palantir-style trailing-lambda layout. Single-block-lambda argument behavior (Rule 8 / single-arg break path) is unchanged. Idempotency holds because the layout is a fixed point of the new branch when the input matches the trailing-lambda pattern. Coverage: `WrappingFormattingTest.trailingBlockLambda_keepsLambdaHeaderOnCallLine_*`.
