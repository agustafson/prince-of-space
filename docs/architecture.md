...
- `modules/cli` (`:cli`) — Command-line tool (picocli); `--java-version N` creates `JavaLanguageLevel.of(N)` which is translated to a JavaParser `LanguageLevel` inside `FormattingEngine` when parsing. The shaded jar is published to Maven Central as `prince-of-space-cli`, primarily so external (non-JVM-build-tool) integrations — e.g. Maven via Spotless's `nativeCmd` step — can fetch it with `maven-dependency-plugin:copy` and invoke `java -jar ... --stdin` (see README "Spotless (Maven)")
- `modules/formatter-benchmark` (`:formatter-benchmark`) — Optional corpus benchmark vs Google Java Format, Palantir Java Format, and Prettier (`./gradlew :formatter-benchmark:run`; see `docs/benchmarks.md`)
- `modules/intellij-plugin` (`:intellij-plugin`) — IntelliJ Platform plugin: **Settings → Tools → Prince of Space** persists full `FormatterConfig` in `.idea/prince-of-space.xml` (see `modules/intellij-plugin/README.md`), optional format-on-save, and language level from the module or a fixed release; **Code → Reformat with Prince of Space…** uses the same configuration. Develop with `./gradlew :intellij-plugin:runIde`, package with `./gradlew :intellij-plugin:buildPlugin`
- `modules/vscode-extension/` — VS Code extension (Node/TypeScript; **not** part of the Gradle multi-project graph — `./gradlew build` does not compile or test it). Build/package from `modules/vscode-extension/` with npm (see its README). Registers a **Java document formatting** provider and **Prince of Space: Format Document**; runs `java -jar` on the **CLI shadow JAR** (`:cli:shadowJar`), resolving `modules/cli/build/libs/prince-of-space-cli-*.jar` from the workspace unless `princeOfSpace.cliJar` is set
