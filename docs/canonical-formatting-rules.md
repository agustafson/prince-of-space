...
- **Trailing block-lambda exception (TDR-021):** when the **last argument** of a wrapped call is a block-bodied lambda, the leading arguments and the lambda header (`() -> {`, `(a, b) -> {`, etc.) stay on the call line, the lambda body wraps with its own block indent, and the closing `})` sits on a single line at the call's indent column **regardless of `closingParenOnNewLine`**. This matches palantir-java-format / Prettier / ktlint conventions and keeps the lambda visually coupled to the call. The opener line may slightly exceed `lineLength` as a soft tradeoff; the alternative (one arg per line with `() ->` on its own line) reads worse. The exception does **not** apply when a leading argument is itself a block-bodied lambda or carries a leading line/block comment — those calls fall back to the standard per-arg wrapping path.
